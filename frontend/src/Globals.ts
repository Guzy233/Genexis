...
// 新的 onSetup 注册系统
export type SetupFn = (canvas: SVGSVGElement) => (() => void) | void;
export const onSetups: SetupFn[] = [];
let canvasInstance: SVGSVGElement | null = null;
const cleanups: (() => void)[] = [];

export function onSetup(setupFn: SetupFn) {
    onSetups.push(setupFn);
    if (canvasInstance) {
        const cleanup = setupFn(canvasInstance);
        if (typeof cleanup === "function") cleanups.push(cleanup);
    }
}

export function setCanvasInstance(canvas: SVGSVGElement) {
    canvasInstance = canvas;
    // 执行所有已注册的 setup
    onSetups.forEach(setup => {
        const cleanup = setup(canvasInstance!);
        if (typeof cleanup === "function") cleanups.push(cleanup);
    });
}

export function clearCanvasInstance() {
